...
do
    ...
    BG.ItemLibMainFrame.Hope = {}
    BG.ItemLibMainFrame.IsVisible = function() return false end
...
do
    ...
    local function SetButtonText(self)
        ...
        if not BG.AddHText(BG.FB1, link, itemID, self) then
            self:SetText(link)
        end
    ...
    function BG.SetListzhuangbei(self)
        ...
        for t = 1, 2 do
            for i = 1, MaxI do
                ...
                local bt = CreateFrame("EditBox", nil, f, BG.editTemplate)
                ...
                if bt.itemID then
                    ...
                    bt:SetScript("OnMouseDown", function(self, button)
                        if self.link then
                            ...
                            -- 已移除「Ctrl+点击跳转装备库」：装备库 UI 在 BGLite 中刻意移除（见 ItemLib.lua 头注释），无目标可跳
                            ...
                            elseif IsAltKeyDown() then
                                ...
                                self.owner:SetText(self.link)
                                self.owner:SetCursorPosition(0)
                                self.owner:ClearFocus()
                                if isHope then
...
do
    function BG.CreateGuanZhuButton(bt, _type)
        ...
        f:SetScript("OnMouseDown", function(self, enter)
            if _type == "biaoge" and enter == "RightButton" then
                BG.SetGuanZhu(bt, 0)
            end
        end)
    ...
    function BG.SetGuanZhu(bt, type)
        local FB = bt.FB
        local b = bt.bossnum
        local i = bt.i
        if type == 1 then
    ...
    function BG.SetBiaoGeGuanZhu(itemID)
        BG.PairFBItem(function(item)
            if GetItemID(item:GetText()) == itemID then
                BG.SetGuanZhu(item, 1)
                return true
            end
        end)
    end
...
function BG.SetListmaijia(maijia, clearFocus, filter, isAuctionLogFrame, onlyName)
...
do
    ...
    function BG.Show_AllHighlight(link, btType)
        ...
        if not btType or btType ~= "biaoge" then
            BG.HighlightBiaoGe(link)
        end
        if not btType or btType ~= "outtime" then
            BG.HighlightItemOutTime(link)
        end
        if not btType or btType ~= "auctionlog" then
...
do
    function BG.CreateScrollFrame(parent, w, h, isEdit, alwaysHide)
        ...
        if isEdit then
            ...
            child:SetFont(BIAOGE_TEXT_FONT, 15, "OUTLINE")
        else
...
function BG.auctionLogFrame_InsertLink(text)
    if BG.auctionLogFrame.changeFrame.zhuangbei:HasFocus() then
        ...
        BG.auctionLogFrame.changeFrame.zhuangbei:SetCursorPosition(0)
    ...
    elseif BG.auctionLogFrame.serachEdit:HasFocus() then
        ...
        BG.auctionLogFrame.serachEdit:SetCursorPosition(0)
        return true
    end
end
...
function BG.InsertLink(text, isZhuangbeiList)
    ...
    local isMyEdit = GetCurrentKeyBoardFocus()
    if not isMyEdit or isZhuangbeiList then
...
function BG.PairFBItem(func, bossNum, all, FB)
    local FB = FB or BG.FB1
    local startB, endB
    if all then
        startB, endB = 1, Maxb[FB] + 2
    elseif bossNum then
        startB, endB = bossNum, bossNum
    else
...
function BG.CreateExportFrame(title, text)
    if BG.freeClearBiaoGeFrame then
