...
do
    ...
    -- Lite: 恢复交易/邮件模块（TradeHistory/MailHistory/SendMail）所需的滚动模板。
    -- 原版 v2.3.5 此处为 "BiaoGe_ModernScrollFrameTemplate" or "UIPanelScrollFrameTemplate"（or 短路恒取前者），
    -- 但 BGLite 已删该自定义模板（Templates.xml），故直接写暴雪内置模板，功能等价、视觉为暴雪默认滚动条。
    ...
    if C_GameRules and C_GameRules.IsHardcoreActive and C_GameRules.IsHardcoreActive() then
        ...
        if not StaticPopupDialogs[frameName] then
            StaticPopupDialogs[frameName] = {
                text = L["BGLite插件提醒：由于暴雪对插件UI数量的限制，本插件无法在硬核服务器使用，请你自行禁用。"],
                button1 = L["好的"],
                OnAccept = function()
                end,
                whileDead = true,
    ...
    -- 与原版 BiaoGe 的共存冲突检测。
    -- 两个插件共用同一个全局表 BG（双方都是 BG = {} 硬重置）与同一个存档表 BiaoGe，
    -- 同时启用会导致后加载者覆盖前者，表现为功能大面积失灵、账本数据交叉污染。
    -- 插件按目录名字母序加载（BGLite < BiaoGe），所以此处只做提示，不自动禁用对方
    -- （不得由插件单方面致哑其它插件）。
    -- 检测必须放在 BG.Init2（PLAYER_ENTERING_WORLD）里：本文件执行时 BiaoGe 尚未加载，
    -- 在这里直接调 IsAddOnLoaded("BiaoGe") 会恒为 false。
    BG.Init2(function()
        if not IsAddOnLoaded("BiaoGe") then return end
        local frameName = 'BiaoGeConflictError'
        if not StaticPopupDialogs[frameName] then
            StaticPopupDialogs[frameName] = {
                text = L["检测到 BiaoGe 插件同时启用。两个插件共用同一存档与同一全局变量，同时启用会导致功能失灵与账本数据混乱，请只保留一个后重新登录。"],
                button1 = L["好的"],
                OnAccept = function()
                end,
                whileDead = true,
...
do
    ...
    -- 数据最小化：这张表会对「其他玩家」（团队成员 / 买家 / 罚款对象 / 交易对方）逐项取值，
    -- 并按 BiaoGe[副本]["boss"..b][<字段名>..i] 写入存档。
    -- 因此只保留记账与显示确实要用到的字段。
    -- 已删除 guild / raceID / guid / factionGroup —— 实测全库 0 个读取点，属纯过度采集；
    -- 其中 guid 是暴雪账号级唯一标识（改名、转服都不变），长期保存无任何功能回报。
    -- 存量存档里的历史字段由下方 BG.Init 里的一次性迁移清除。
    BG.playerClass = {
        class = { func = UnitClass, select = 2 },               -- 职业（职业图标与颜色推导）
        level = { func = UnitLevel, select = 1 },               -- 等级（显示）
        realm = { func = UnitRealm, select = 1 },               -- 服务器（跨服名字后缀）
        color = { func = UnitColor, select = 1 },               -- 颜色（买家名字着色）
    }
    -- 曾经采集、现已停止采集的字段名，供一次性迁移清理存量数据使用
    BG.removedIdentityFields = { "guild", "raceID", "guid", "factionGroup" }
...
BG.Init(function()
    ...
    --  ① 原版与 BGLite_v3 加载时对 History 只「创建/迁移、永不删除」，无条件置 nil 是本改造引入的偏差；
    ...
    -- 注：BGLite 未恢复 BG.CreateDisableButton 禁用按钮 UI，此表仅作守卫初始化，恒为启用。
    BiaoGe.disabledModules = BiaoGe.disabledModules or {}

    -- 一次性迁移：清除存量存档里已停止采集的身份字段（键形如 guid1 / raceID2 / factionGroup3）。
    -- 这些字段全库 0 个读取点，删除不影响任何功能；其中 guid 是暴雪账号级唯一标识，
    -- 改名转服都不变，不应连同金额长期留在账本里。
    -- 采用「键名模式匹配 + 递归」而不是按副本/BOSS/行号遍历，以免因结构假设错误而漏行。
    -- 只匹配「字母 + 数字后缀」形式，因此不会误删 BiaoGe.playerInfo 下的同名字段（那些无数字后缀，
    -- 且只记录本机自己的角色，是正当用途）。
    if not BiaoGe.migrated_dropIdentityFields then
        local drop = {}
        for _, key in ipairs(BG.removedIdentityFields or {}) do
            drop[key] = true
        end
        local function purge(t, depth)
            if depth > 10 or type(t) ~= "table" then return end
            for k, v in pairs(t) do
                local name = type(k) == "string" and k:match("^(%a+)%d+$")
                if name and drop[name] then
                    t[k] = nil
        ...
        purge(BiaoGe, 1)
        BiaoGe.migrated_dropIdentityFields = true
    end
