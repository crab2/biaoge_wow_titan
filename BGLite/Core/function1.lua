...
function BG.SendSystemMessage(msg)
    SendSystemMessage(BG.STC_b1("<bg次bis版>") .. " " .. msg)
end
...
-- 「最佳价格」由第三方付费模块 BGV 提供，本插件不做付费模块挂载点，故恒返回 false。
-- 保留空实现是为了让各调用点（表格/背包/拍卖记录的点击分支）无需逐处改写即可短路。
...
-- 输入框边框着色原由第三方付费模块 BGV 提供，已移除该挂载点，此处不做任何处理。
